...
export const getApplyList = async () => {
  return request.get("/api/friend/apply-list");
};

//删除好友申请
export const deleteApply = async (id) => {
  return request.delete(`/api/friend/delete-apply/${id}`);
};

//查找好友
export const getSearchList = async (keywords) => {
  return request.get(`/api/friend/search/${keywords}`);
};

//申请好友确认界面
export const showConfirm = async (data) => {
  return request.post("/api/friend/show-confirm", data);
};
...
export const postVerify = async (data) => {
  return request.post("/api/friend/verify", data);
};

//更新好友
export const putUpdate = async (data) => {
  return request.put("/api/friend/update", data);
};
